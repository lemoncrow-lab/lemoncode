...
const id = "internal:sidebar-lemoncrow"

// Written by the LemonCrow gateway after every turn; the path is exported by
// `lc code` (see core/capabilities/statusline_sidecar.py).
  ...
  return (
    <Show when={has()}>
      <box>
        ...
        <text fg={theme().textMuted}>{tokens(snapshot()?.context_tokens)} ctx</text>
        <Show when={(snapshot()?.cache_efficiency_pct ?? 0) > 0}>
          <text fg={theme().textMuted}>{Math.round(snapshot()?.cache_efficiency_pct ?? 0)}% cached</text>
        </Show>
        <text fg={theme().textMuted}>{money.format(snapshot()?.cost_usd ?? 0)} spent</text>
        <Show when={(snapshot()?.saved_usd ?? 0) > 0}>
          <text fg={theme().success}>{money.format(snapshot()?.saved_usd ?? 0)} saved</text>
        </Show>
